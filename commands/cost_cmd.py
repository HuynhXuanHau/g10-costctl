...
def run(args):
    ...
    # Print header
    print(f"Cost for {key}={val} over last {days} days ({start_s} → {end_s}):")
    print("    " + "-" * 60)
    for svc, amt in sorted(sums.items(), key=lambda kv: kv[1], reverse=True):
      print(f"  {svc:40} ${amt:8.2f}")
    print("    " + "-" * 60)
    print(f"  TOTAL{(40-5)*' '} ${total:8.2f}")
