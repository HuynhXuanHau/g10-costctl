...
def _tag_rds(rid, tags):
    ...
    arn = resp.get("DBInstances", [])[0].get("DBInstanceArn")
...
def _tag_s3(rid, tags):
    ...
    try:
        ...
        existing = []
    # merge: existing -> dict, then update with new tags
    m = {t["Key"]: t["Value"] for t in existing}
    for t in tags:
        m[t["Key"]] = t["Value"]
    merged = [{"Key": k, "Value": v} for k, v in m.items()]
